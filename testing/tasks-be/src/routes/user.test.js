...
describe("user", () => {
  ...
  it("should create a user correctly", async () => {
    ...
    const res = await clonServer(app).post("/users/signup").send(user);
  ...
  it("should not create user when email already exists", async () => {
    const user = { email: "test@test.com", password: "12345" };
    // await clonServer(app).post('/users/signup').send(user)
  ...
  it("should signin user correctly", async () => {
    const user = { email: "test@test.com", password: "12345" };
    await User.create(user);
    const res = await clonServer(app).post("/users/signin").send(user);
